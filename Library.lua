...
-- Treat both a mouse click and a touch tap as "pressed" so every
-- draggable/slidable control also works on phones and tablets.
local function isPress(input)
	return input.UserInputType == Enum.UserInputType.MouseButton1
		or input.UserInputType == Enum.UserInputType.Touch
end

-- Measures how tall a chunk of text needs to be once it wraps inside
-- a given pixel width, so dropdown entries (and anything similar) can
-- grow instead of clipping long words/phrases onto one line.
local function getWrappedHeight(text, font, textSize, width)
	local size = TextService:GetTextSize(text, textSize, font, Vector2.new(math.max(width, 20), math.huge))
	return math.max(20, size.Y + 6)
end

local function sumItemHeights(objects)
	local total = 0
	for i, v in pairs(objects:GetChildren()) do
		if not v:IsA("UIListLayout") then
			total = total + v.AbsoluteSize.Y
		end
	end
	return total
end
...
-- On touch devices there's no RightShift key to press, so give mobile
-- players a small draggable on-screen button to open/close the menu.
if isMobile then
	local mobileToggle = Instance.new("ImageButton")
	mobileToggle.Name = "MobileToggle"
	mobileToggle.Parent = imgui
	mobileToggle.Position = UDim2.new(0, 10, 0, 120)
	mobileToggle.Size = UDim2.new(0, 46, 0, 46)
	mobileToggle.BackgroundTransparency = 1
	mobileToggle.Image = "rbxassetid://2851926732"
	mobileToggle.ImageColor3 = ui_options.main_color
	mobileToggle.ScaleType = Enum.ScaleType.Slice
	mobileToggle.SliceCenter = Rect.new(12, 12, 12, 12)
	mobileToggle.ZIndex = 10000
...
function library:AddWindow(title, options)
	...
	-- Shrink the whole window (and every descendant, since UIScale scales
	-- all children) to fit small phone screens instead of overflowing them.
	if isMobile then
		local uiScale = Instance.new("UIScale")
		uiScale.Parent = Window
		local viewport = (workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize) or Vector2.new(800, 600)
		uiScale.Scale = math.clamp(viewport.X / 900, 0.55, 1)
	end
	...
	-- Bigger touch target for the resize handle on phones/tablets.
	if isMobile then
		Resizer.Size = UDim2.new(0, 34, 0, 34)
		Resizer.Position = UDim2.new(1, -34, 1, -34)
	end
	...
	do -- Resize Window
		...
		-- Driven off Resizer.InputBegan instead of a global listener +
		-- hover flag, so a touch tap that starts on the handle works
		-- exactly like a mouse press starting on it.
		local Held = false
		Resizer.InputBegan:Connect(function(inputObject)
			if isPress(inputObject) and Resizer.Active and options.can_resize then
				Held = true
				Window.Draggable = false
				...
				do -- Tab Elements
					...
					function tab_data:AddSlider(slider_text, callback, slider_options)
						...
						do -- Slider Math
							...
							-- Driven off slider.InputBegan (fires for mouse
							-- press AND touch tap on the control itself) so
							-- a quick tap-and-drag works without needing a
							-- MouseEnter that touch never sends.
							local Held = false
							slider.InputBegan:Connect(function(inputObject)
								if isPress(inputObject) and not slider_options.readonly then
									Held = true
									Window.Draggable = false
					...
					function tab_data:AddDropdown(dropdown_name, callback)
						...
						-- Thicker scrollbar so it's easy to grab with a finger.
						if isMobile then
							objects.ScrollBarThickness = 14
						end

						-- Cap how tall the open list can get before it scrolls,
						-- so a dropdown with many long entries doesn't take
						-- over the whole screen.
						local MAX_VISIBLE = 220

						local open = false
						dropdown.MouseButton1Click:Connect(function()
							open = not open

							local full = sumItemHeights(objects)
							local len = math.min(full, MAX_VISIBLE)
							objects.CanvasSize = UDim2.new(0, 0, 0, full)
						...
						function dropdown_data:Add(n)
							...
							-- Let long entries wrap onto multiple lines instead
							-- of clipping, and size the item to fit the wrapped
							-- text so nothing overlaps.
							object.TextWrapped = true
							object.TextXAlignment = Enum.TextXAlignment.Left
							object.TextYAlignment = Enum.TextYAlignment.Top

							local width = (box.AbsoluteSize.X > 0 and box.AbsoluteSize.X) or dropdown.AbsoluteSize.X
							local extraTouchPadding = isMobile and 8 or 0
							local h = getWrappedHeight(object.Text, object.Font, object.TextSize, width - 20) + extraTouchPadding
							object.Size = UDim2.new(1, 0, 0, h)
					...
					function tab_data:AddColorPicker(callback)
						...
						do -- Color Picker Math
							...
							-- Driven off InputBegan on each control directly
							-- (rather than a global listener + hover flag) so
							-- a touch tap that starts on the palette/strip
							-- works the same way a mouse press does.
							local Held = false
							local heldTarget = nil -- "palette" or "saturation"
							...
							palette.InputBegan:Connect(function(inputObject)
								if isPress(inputObject) then
									...
									spawn(function()
										while Held and heldTarget == "palette" and (not dropdown_open) do -- Palette
											local mouse_location = gMouse()
							...
							saturation.InputBegan:Connect(function(inputObject)
								if isPress(inputObject) then
									Held = true
									heldTarget = "saturation"
									Window.Draggable = false

									spawn(function()
										while Held and heldTarget == "saturation" and (not dropdown_open) do -- Saturation
											local mouse_location = gMouse()
											local y = ((palette.AbsoluteSize.Y - (mouse_location.Y - palette.AbsolutePosition.Y)) + 1.5)
											v = y / 100
